...
class LectureService(
    ...
    fun pageByKeyword(
        ...
        val lectures = lectureRepository.getByKeywordWithCursor(
            keyword = keyword,
            nextId = nextId,
            limit = limit + 1,
        )
        ...
        val schedulesByLectureId = lectureRepository.getSchedulesByLectureIds(lectureIds)
            .groupBy { it.lectureId }

        val lectureDtos = lectures.map { lecture ->
            LectureDto(
                id = lecture.id!!,
                ...
                schedule = schedulesByLectureId[lecture.id]?.map {
                    LectureSchedule(
